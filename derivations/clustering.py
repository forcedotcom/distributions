...
from ccdb.util import json_stream_load, json_stream_dump
...
def test_enum_counts(max_size=8):
    all_counts = enum_counts(max_size)
    for size in range(1, 1 + max_size):
        parts = ccdb.enumerate.enumerate_partitions(range(size))
        counts = defaultdict(lambda: 0)
        for part in parts:
            counts[tuple(sorted(map(len, part)))] += 1
        counts = dict(counts)
        expected = counts
        actual = all_counts[size]
...
@parsable.command
def approximations(max_size=50):
    '''
...
def code(max_size=47):
    ...
    lines = [
        '// this code generated by clustering.py',
        'static const float cluster_normalizing_scores[%d] =' % (max_size + 1),
        '{',
        ',\n'.join(['    %0.8ff' % x for x in log_Z]),
        '};',
        '',
        '// this code generated by clustering.py',
        'inline float cluster_normalizing_score(int n)',
        '{',
        '    if (n < %d) {' % (max_size + 1),
        '        return cluster_normalizing_scores[n];',
        '    } else {',
        '        float coeff = %0.8ff;' % coeff,
        '        float log_z_max = n * fastlog(n);',
