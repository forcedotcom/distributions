...
CPP_SOURCES:=$(shell find include src examples benchmarks | grep -v 'vendor\|\.pb\.' | grep '\.\(cc\|hpp\)$$')

lint_cc: FORCE
	echo $(CPP_SOURCES)
	cpplint --filter=-build/include_order,-readability/streams $(CPP_SOURCES) 2>&1
